// ============================================================
//  CHAT FLUTUANTE v9.0 — Leandro Bomfim Imóveis (dock + WhatsApp; paleta única; NLP factual reforçado)
...
(function () {
    ...
    // Expande abreviações, gírias cariocas e informais antes de detectar intenção
    function expandAbbr(t) {
        const abbr = {
            ...
            // Gírias cariocas e informais do RJ
            ' treta ': ' problema ', ' mano ': ' amigo ', ' cara ': ' pessoa ',
            ' bicho ': ' amigo ', ' parceiro ': ' amigo ', ' papo ': ' conversa ',
            ' da hora ': ' otimo ', ' irado ': ' otimo ', ' massa ': ' otimo ',
            ' trampo ': ' trabalho ', ' rolo ': ' negocio ', ' negocio ': ' negocio ',
            ' firmeza ': ' certo ', ' se liga ': ' atencao ', ' minha filha ': ' voce ',
            ' meu filho ': ' voce ', ' escuta ': ' ouvi ', ' oi gente ': ' ola ',
            ' oi pessoal ': ' ola ', ' salve ': ' ola ', ' eita ': ' surpresa ',
            ' oxe ': ' surpresa ', ' pois e ': ' concordo ', ' tipo ': ' como ',
            ' po ': ' poxa ', ' pow ': ' poxa ', ' saca ': ' entendeu ',
            ' topo ': ' quero ', ' topei ': ' quero ', ' bora ': ' vamos ',
            ' vamo ': ' vamos ', ' ta bom ': ' concordo ', ' ta otimo ': ' concordo ',
            ' tamo ': ' estamos ', ' tamu ': ' estamos ', ' solteiro ': ' disponivel ',
            ' kd ': ' onde esta ', ' qd ': ' quando ', ' hj ': ' hoje ',
            ' mermao ': ' amigo ', ' mermo ': ' amigo ',
            ' coe ': ' o que ', ' ce ': ' voce ',
            ' papo reto ': ' sinceridade ', ' na moral ': ' sinceramente ',
            ' naquele pique ': ' bem ', ' ta ligado ': ' entende ',
            ' sinistro ': ' muito bom ', ' brabo ': ' excelente ', ' brabissimo ': ' excelente ',
            ' qual foi ': ' o que foi ', ' suave ': ' tranquilo ', ' joia ': ' otimo ',
            ' resenha ': ' conversa ', ' zuera ': ' brincadeira ', ' zoeira ': ' brincadeira ',
            ' na faixa ': ' barato ', ' pancada ': ' absurdo ', ' absurdo ': ' muito ',
            ' amanha ': ' amanha ', ' agr ': ' agora ', ' dps ': ' depois ',
            ' mds ': ' meu deus ', ' sds ': ' saudades ', ' n ': ' nao ',
            ' num ': ' nao ', ' nunca ': ' nunca ', ' jamais ': ' nunca ',
            ' tmj ': ' juntos ', ' slk ': ' serio ', ' smk ': ' serio ',
            ' zap ': ' whatsapp ', ' wpp ': ' whatsapp ', ' zap zap ': ' whatsapp ',
            ' insta ': ' instagram ', ' face ': ' facebook ',
            ' rj ': ' rio de janeiro ', ' rio ': ' rio de janeiro ',
            ' zona s ': ' zona sul ', ' brt ': ' brt transporte ',
            // Termos imobiliários informais cariocas
            ' pe na areia ': ' perto da praia ', ' de frente pro mar ': ' vista mar ',
            ' tava com o leandro ': ' falar com leandro ', ' de ripe ': ' reformado ',
            ' em cima ': ' andar superior ', ' anda ': ' andares ',
            ' metragem ': ' area ', ' p ': ' para ', ' pra ': ' para ',
            ' meia ': ' 500 ', ' conto ': ' mil reais ', ' mil ': ' mil reais ',
        };
        let r = ' ' + t + ' ';
    ...
    // ── Distância de Levenshtein (calculadora de erros de digitação) ──
    ...
    // Verifica se o texto normalizado contém a palavra (com tolerância a erros robusta)
    function fuzzySearch(text, pattern) {
        ...
        // Trata match com multiplas palavras no pattern
        if (pattern.includes(' ')) {
            const pWords = pattern.split(' ').length;
            for (let i = 0; i <= words.length - pWords; i++) {
                textTokens.push(words.slice(i, i + pWords).join(' '));
            }
        } else {
            textTokens = words;
        }

        // Permite até 2 erros tipográficos para palavras com 8 ou mais letras, ou 1 erro para palavras menores
        const maxDist = Math.max(1, Math.floor(pattern.length * 0.25));

        for (const token of textTokens) {
            // Verifica token absoluto contra o padrao
            if (levenshtein(token, pattern) <= maxDist) return true;
            // Se o token continha sujeira a mais anexada, tenta com includes simples (fallback da antiga busca)
            if (token.includes(pattern)) return true;
        }
        return false;
    }

    // Verifica se QUALQUER padrão da lista está no texto tolerando erros
    function matchAny(text, patterns) {
        const t = normFull(text);
        return patterns.some(p => {
            const np = norm(p);
            return t.includes(np) || fuzzySearch(t, np);
        });
    }

    // Verifica se TODOS os padrões estão presentes tolerando erros
    ...
    /** Imóvel do modal, último visto ou único anúncio no catálogo */
    function _imovelRefCatalogo() {
        return window.lbActiveImovel || window.lbLastImovel || (_catalogoImoveis.length === 1 ? _catalogoImoveis[0] : null);
    }

    /** Resposta factual: um imóvel ou listagem resumida (vários anúncios) */
    function _respostaCampoCatalogo(cfg) {
        const { emoji, nome, field, waFrase, parseVal } = cfg;
        if (!_catalogoLoaded) {
            return {
                text: `⏳ Catálogo carregando… Tente de novo em instantes ou pergunte no WhatsApp sobre **${nome}**.`,
                opts: [{ label: '📱 WhatsApp', action: () => window.open(KB.wa(waFrase), '_blank') }, { label: '↩ Menu', next: 'inicio' }],
            };
        }
        const im = _imovelRefCatalogo();
        if (im) {
            const raw = im[field];
            const ok = parseVal ? parseVal(raw, im) : (raw != null && String(raw).trim() !== '');
            const exib = ok ? (typeof cfg.format === 'function' ? cfg.format(raw, im) : String(raw)) : null;
            const txt = ok
                ? `${emoji} No imóvel **${im.titulo || 'selecionado'}**: **${exib}** (${nome}).`
                : `${emoji} No imóvel **${im.titulo || 'selecionado'}** o campo **${nome}** não está preenchido no site. O Leandro confirma na ficha.`;
            return {
                text: txt,
                opts: [
                    { label: '📱 Confirmar com Leandro', action: () => window.open(KB.wa(`${waFrase} (*${im.titulo}*).`), '_blank') },
                    { label: '🏠 Ver no site', action: () => { window.location.href = 'imoveis.html'; } },
                    { label: '↩ Menu', next: 'inicio' },
                ],
            };
        }
        if (!_catalogoImoveis.length) {
            return { text: '📋 Não há imóveis listados agora.', opts: [{ label: '📱 Falar com Leandro', action: () => window.open(KB.whatsapp, '_blank') }, { label: '↩ Menu', next: 'inicio' }] };
        }
        const lines = _catalogoImoveis.slice(0, 8).map((x) => {
            const raw = x[field];
            const ok = parseVal ? parseVal(raw, x) : (raw != null && String(raw).trim() !== '');
            const exib = ok ? (typeof cfg.format === 'function' ? cfg.format(raw, x) : String(raw)) : 'não informado no site';
            return `• **${x.titulo || 'Imóvel'}** — ${exib}`;
        }).join('\n');
        const more = _catalogoImoveis.length > 8 ? `\n\n_…e mais ${_catalogoImoveis.length - 8}._` : '';
        return {
            text: `${emoji} **${_catalogoImoveis.length} imóveis** no catálogo — **${nome}**:\n\n${lines}${more}`,
            opts: [
                { label: '🌐 Abrir Imóveis', action: () => { window.location.href = 'imoveis.html'; } },
                { label: '📱 WhatsApp', action: () => window.open(KB.wa(waFrase), '_blank') },
                { label: '↩ Menu', next: 'inicio' },
            ],
        };
    }
    ...
    function detectIntent(rawText) {
        ...
        // ── Continuidade: confirmação / negação / pedir detalhe (sem API de IA) ──
        if (_ctx.lastIntent && t.length <= 42) {
            const affirm = matchAny(t, ['sim', 'sii', 'isso', 'exato', 'aham', 'ok', 'okay', 'certo', 'beleza', 'perfeito', 'pode ser', 'claro', 'com certeza', 'quero sim', 'bora', 'vamos', 'manda', 'show', 'legal', 'isso mesmo', 'exatamente', 'uhum', 'positivo']);
            const deny = matchAny(t, ['nao', 'não', 'nada disso', 'outra coisa', 'deixa', 'esquece', 'cancela', 'prefiro nao', 'prefiro não', 'outro assunto', 'mudando de assunto']);
            const clarify = matchAny(t, ['nao entendi', 'não entendi', 'explica', 'explica melhor', 'como assim', 'mais detalhes', 'o que e isso', 'o que é isso', 'me explica', 'nao sei', 'não sei', 'nao sei nao', 'não sei não']);
            if (clarify) {
                if (['FINANCIAMENTO', 'FGTS', 'ENTRADA', 'JUROS', 'APROVACAO', 'MCMV', 'COMO_COMPRAR', 'DOCUMENTACAO', 'CUSTOS_EXTRAS', 'CONDOMINIO', 'REFORMA'].includes(_ctx.lastIntent))
                    return { intent: _ctx.lastIntent };
                if (_ctx.lastBairroKey) return { intent: 'INFO_BAIRRO', bairro: _ctx.lastBairroKey };
            }
            if (affirm && ['FINANCIAMENTO', 'FGTS', 'ENTRADA', 'AGENDAR_VISITA', 'DOCUMENTACAO', 'CUSTOS_EXTRAS', 'COMO_COMPRAR', 'JUROS', 'APROVACAO', 'MCMV', 'CONDOMINIO', 'SEGURANCA_COMPRA', 'VENDER', 'AVALIACAO'].includes(_ctx.lastIntent))
                return { intent: 'CONTATO_GERAL' };
            if (deny) return { intent: 'CATALOGO_GERAL' };
        }

        // ── Continuidade pelo último fluxo (menu/chips) — sem API de IA ──
        if (t.length < 100 && _ctx.lastFlow) {
            if ((_ctx.lastFlow === 'flow_catalogo' || _ctx.lastIntent === 'CATALOGO_GERAL' || _ctx.lastIntent === 'BUSCA_ESPECIFICA') &&
                matchAny(t, ['preco', 'precos', 'valor', 'valores', 'quanto custa', 'quanto e', 'financi', 'parcela', 'prestacao', 'taxa', 'banco'])) {
                return { intent: 'FINANCIAMENTO' };
            }
            if ((_ctx.lastFlow === 'flow_catalogo' || _ctx.lastIntent === 'CATALOGO_GERAL') &&
                matchAny(t, ['itbi', 'cartorio', 'documento', 'documentacao', 'escritura', 'papelada', 'matricula'])) {
                return { intent: 'DOCUMENTACAO' };
            }
            if ((_ctx.lastFlow === 'flow_catalogo' || _ctx.lastIntent === 'CATALOGO_GERAL' || _ctx.lastIntent === 'BUSCA_ESPECIFICA') &&
                matchAny(t, ['visita', 'agendar', 'conhecer pessoalmente', 'ir ver', 'marcar', 'horario para ver'])) {
                return { intent: 'AGENDAR_VISITA' };
            }
            if (_ctx.lastFlow === 'flow_financiamento' && matchAny(t, ['fgts', 'fundo de garantia', 'usar o fgts', 'usar fgts'])) {
                return { intent: 'FGTS' };
            }
            if (_ctx.lastFlow === 'flow_financiamento' && matchAny(t, ['entrada', 'valor de entrada', 'quanto de entrada'])) {
                return { intent: 'ENTRADA' };
            }
        }
        if (t.length < 80 && (_ctx.lastIntent === 'LEANDRO_HISTORIA' || _ctx.lastIntent === 'LEANDRO_DIFERENCIAL') &&
            matchAny(t, ['creci', 'registro', 'e legal', 'confio'])) {
            return { intent: 'LEANDRO_CRECI' };
        }

        // ── NOME DO USUÁRIO ──
        const nameMatch = t.match(/(?:meu nome e|me chamo|sou o|sou a|pode me chamar de)\s+([a-z]{2,})/);
        if (nameMatch && !_ctx.userName) _ctx.userName = nameMatch[1].charAt(0).toUpperCase() + nameMatch[1].slice(1);

        // ── IMÓVEL ATIVO NO MODAL OU ÚLTIMO VISTO ──
        const isContextActiveModal = matchAny(t, ['este imovel', 'esse imovel', 'aqui', 'da foto', 'deste', 'desse', 'desse anuncio', 'deste anuncio', 'que esta na tela', 'nesse que estou']);
        // Se o usuário está com um imóvel aberto (window.lbActiveImovel existe) ou recém-visto (window.lbLastImovel):
        const activeModalImovel = window.lbActiveImovel || window.lbLastImovel || null;
        if (activeModalImovel) {
            if (isContextActiveModal || matchAny(t, ['quartos', 'quarto', 'suites', 'suite', 'banheiros', 'banheiro', 'vagas', 'vaga', 'garagem', 'tamanho', 'area', 'm2', 'condominio', 'iptu', 'documentacao', 'reforma', 'preco', 'valor', 'custa', 'visita', 'agendar'])) {
                if (!_extractBairro(t) && !matchAny(t, ['buscar', 'outros', 'tem mais', 'mais opcoes'])) {
                    return { intent: 'INFO_IMOVEL_ATIVO', imovel: activeModalImovel, raw: t };
                }
            }
        }

        // ── Contagem direta no catálogo (antes de saudações — evita colisão com substrings) ──
        const asksImovelCount =
            matchAny(t, [
                'tem quantos imoveis', 'tem quantos imovel', 'quantos imoveis tem', 'quantos imoveis voce tem',
                'quantos imoveis vc tem', 'quantos imoveis tem no', 'quantos imoveis no catalogo', 'quantos imoveis no site',
                'quantos no catalogo', 'quantos no site', 'numero de imoveis', 'total de imoveis',
            ])
            || (t.includes('quantos imoveis') && !matchAny(t, ['quarto', 'quartos', 'suite', 'suites', 'banheiro', 'banheiros', 'metro', 'andar']));
        if (asksImovelCount) return { intent: 'CONTAGEM_IMOVEIS' };

        if (matchAny(t, ['banheiro', 'banheiros', 'lavabo'])
            && matchAny(t, ['quantos', 'quantas', 'qtos', 'qtd', 'tem quantos', 'qual o numero', 'numero de', 'quantidade'])) {
            return { intent: 'INFO_BANHEIROS_AGG' };
        }
        if (matchAny(t, ['quarto', 'quartos', 'dormitorio', 'dormitorios', 'dorm'])
            && matchAny(t, ['quantos', 'quantas', 'qtos', 'qtd', 'tem quantos', 'qual o numero', 'numero de', 'quantidade', 'tem quanto'])) {
            return { intent: 'INFO_QUARTOS_AGG' };
        }
        if (matchAny(t, ['suite', 'suites'])
            && matchAny(t, ['quantos', 'quantas', 'qtos', 'qtd', 'tem quantos', 'qual o numero', 'numero de', 'quantidade'])) {
            return { intent: 'INFO_SUITES_AGG' };
        }
        if (matchAny(t, ['vaga', 'vagas', 'garagem'])
            && (matchAny(t, ['quantos', 'quantas', 'qtos', 'numero de', 'quantidade']) || matchAny(t, ['tem vaga', 'tem garagem', 'inclui vaga', 'possui vaga']))) {
            return { intent: 'INFO_VAGAS_AGG' };
        }
        if (matchAny(t, ['metragem', 'metros quadrados', 'm2', 'area util', 'area do imovel', 'tamanho do imovel', 'tamanho'])
            && matchAny(t, ['qual', 'quanto', 'quantos', 'quantas', 'e a', 'tem quanto', 'tem quanta', 'numero'])) {
            return { intent: 'INFO_AREA_AGG' };
        }
        if ((matchAny(t, ['qual o preco', 'qual o valor', 'preco do imovel', 'valor do imovel', 'quanto custa o imovel', 'quanto e o imovel', 'preco deste', 'valor deste', 'quanto esta'])
            || (matchAny(t, ['quanto custa', 'quanto e', 'qual valor']) && matchAny(t, ['no catalogo', 'no site', 'imovel listado', 'esse imovel', 'este imovel'])))
            && !matchAny(t, ['condominio', 'iptu', 'taxa', 'parcela', 'financi', 'entrada', 'banco'])) {
            return { intent: 'INFO_PRECO_AGG' };
        }

        // ── Assistente vs humano / o que você faz ──
        if (matchAny(t, [
            'voce e um robo', 'vc e um robo', 'voce e robo', 'e um bot', 'e bot', 'fala com humano',
            'quero falar com humano', 'atendente humano', 'e ia', 'inteligencia artificial', 'voce e ia',
            'vc e ia', 'quem e voce', 'o que e voce', 'voce e real',
            // O que você faz / capacidades
            'o que voce faz', 'o que vc faz', 'o que voce pode fazer', 'o que vc pode fazer',
            'para que voce serve', 'pra que voce serve', 'para que vc serve', 'pra que vc serve',
            'como voce pode me ajudar', 'como vc pode me ajudar',
            'quais sao suas funcoes', 'quais suas funcoes', 'quais suas capacidades',
            'o que posso perguntar', 'o que posso te perguntar', 'o que posso falar',
            'como funciona esse chat', 'como funciona esse assistente', 'como funciona o chat',
            'o que voce sabe fazer', 'o que vc sabe fazer', 'o que voce sabe', 'o que vc sabe',
            'me explica o que voce e', 'me explica o que vc e', 'o que e isso aqui',
            'qual e a sua funcao', 'qual sua funcao', 'voce serve pra que',
            'vc serve pra que', 'pra que serve esse chat', 'para que serve esse chat',
            'o que voce oferece', 'o que vc oferece', 'o que tem aqui',
            'me ajuda com o que', 'voce pode me ajudar com o que', 'vc pode me ajudar com o que',
            'qual sua especialidade', 'quais sao seus recursos', 'o que esse chat faz',
        ])) return { intent: 'ASSISTENTE_QUEM_E' };
        ...
        // ── SAUDAÇÕES ── (evite padrões muito curtos que casam dentro de "imoveis", ex.: "ei" em imóveis)
        if (matchAny(t, [
            'oi ', 'ola ', 'bom dia', 'boa tarde', 'boa noite', 'tudo bem', 'tudo bom',
            'hey ', 'hello', 'salve', 'eae', 'e ai', 'opa ', 'oie ', 'como vai',
            'como voce esta', 'como vc ta', 'ta bom', 'tudo certo', 'beleza', 'fala ai',
            'boa', 'fala comigo', 'to aqui', 'iae', 'yo ', 'hi ', 'whats up',
            // Gírias cariocas / informais RJ
            'iae mano', 'iae cara', 'oi gente', 'oi pessoal', 'ola gente',
            'fala mano', 'fala cara', 'fala parceiro', 'tamo junto', 'tmj',
            'boa tarde mano', 'bom dia mano', 'boa noite mano',
            'eita', 'ae', 'e nois', 'firmeza', 'bora', 'oi sumido', 'oi sumida',
            'quero umas dicas', 'precisando de ajuda', 'to precisando',
            'mermao', 'coe ai', 'coe mano', 'coe ', 'papo reto', 'qual foi', 'suave',
            'na moral', 'beleza ai', 'tranquilo', 'ligado', 'fala mermao', 'salve familia',
            'e o brabo', 'nois', 'noiz', 'rapaziada', 'galera',
        ]) || raw === 'oi' || raw === 'ola' || raw === 'hey' || raw === 'opa' || raw === 'boa' || raw === 'eae' || raw === 'ae' || raw === 'e nois' || raw === 'coe' || raw === 'coé') return { intent: 'SAUDACAO' };
        ...
        // ── CONVERSACIONAIS CURTAS — intenções implícitas comuns ──
        if (matchAny(t, [
            'quero ver', 'quero ver mais', 'me mostra mais', 'tem mais', 'mostra mais',
            'mais opcoes', 'outras opcoes', 'quero outras opcoes', 'ver mais',
            'algo mais', 'tem outras', 'tem outra opcao', 'o que mais tem',
        ])) return { intent: 'CATALOGO_GERAL' };

        if (matchAny(t, [
            'gostei', 'gostei desse', 'gostei desse imovel', 'adorei', 'me interessei',
            'quero esse', 'quero saber mais desse', 'tenho interesse', 'tenho interesse nesse',
            'me chama atencao', 'achei legal', 'achei bom', 'esse me interessa',
        ])) return { intent: 'AGENDAR_VISITA' };

        if (matchAny(t, [
            'preciso de ajuda', 'pode me ajudar', 'ajuda', 'socorro', 'nao sei',
            'nao sei o que quero', 'estou perdido', 'perdido', 'me ajude',
            'como começo', 'por onde começo', 'nao entendo', 'nao entendi',
        ])) return { intent: 'GREETING' };

        if (matchAny(t, [
            'sim', 'ok', 'vamos', 'bora', 'pode ser', 'manda', 'claro', 'com certeza',
            'quero sim', 'sim quero', 'afirmativo', 'manda ver', 'show', 'legal',
        ])) return { intent: 'CATALOGO_GERAL' };
        ...
        if (matchAny(t, [
            ...
            'qual imovel ta disponivel', 'qual imovel tem disponivel', 'qual ta disponivel',
            'tem algo pra vender', 'tem casa para vender', 'tem apartamento a venda',
        ...
        // ── INVESTIMENTO / AIRBNB → redireciona para WhatsApp ──
        if (matchAny(t, [
        ...
        ])) return { intent: 'CONSULTAR_LEANDRO' };
    ...
    function gerarResposta(intentObj) {
        ...
        if (intent === 'BUSCA_ESPECIFICA') {
            ...
            if (!lista.length) return {
                text: `🔍 Não encontrei imóveis com **${filtros}** no catálogo agora.\n\nMas o **Leandro** tem acesso a opções exclusivas! Pergunte a ele diretamente.`,
                opts: [
        ...
        switch (intent) {
            case 'INFO_IMOVEL_ATIVO':
                return _buildModalContextResponse(intentObj.imovel, intentObj.raw);

            case 'CONTAGEM_IMOVEIS': {
                if (!_catalogoLoaded) {
                    return {
                        text: '⏳ Estou carregando o catálogo… Em instantes consigo te dizer o total. Enquanto isso, abra a página **Imóveis**.',
                        opts: [{ label: '🏠 Abrir Imóveis', action: () => { window.location.href = 'imoveis.html'; } }, { label: '↩ Menu', next: 'inicio' }],
                    };
                }
                const n = _catalogoImoveis.length;
                return {
                    text: `📊 Hoje o site lista **${n} imóvel${n !== 1 ? 'is' : ''}** no catálogo online.\n\nQuer ver por bairro, tipo ou abrir a listagem completa?`,
                    opts: [
                        { label: '📍 Ver bairros', next: 'flow_bairros' },
                        { label: '🏠 Ver catálogo', next: 'flow_catalogo' },
                        { label: '🌐 Abrir página Imóveis', action: () => { window.location.href = 'imoveis.html'; } },
                        { label: '↩ Menu', next: 'inicio' },
                    ],
                };
            }

            case 'INFO_BANHEIROS_AGG':
                return _respostaCampoCatalogo({
                    emoji: '🚿', nome: 'banheiros', field: 'banheiros',
                    waFrase: 'Olá Leandro! Quero confirmar banheiros/lavabos do imóvel',
                    parseVal: (v) => v != null && String(v).trim() !== '' && !Number.isNaN(parseInt(v, 10)),
                    format: (v) => `${parseInt(v, 10)} banheiro(s)`,
                });

            case 'INFO_QUARTOS_AGG':
                return _respostaCampoCatalogo({
                    emoji: '🛏️', nome: 'quartos', field: 'quartos',
                    waFrase: 'Olá Leandro! Quero confirmar quantos quartos tem o imóvel',
                    parseVal: (v) => v != null && String(v).trim() !== '' && parseInt(v, 10) >= 0,
                    format: (v) => `${parseInt(v, 10)} quarto(s)`,
                });

            case 'INFO_SUITES_AGG':
                return _respostaCampoCatalogo({
                    emoji: '🛀', nome: 'suítes', field: 'suites',
                    waFrase: 'Olá Leandro! Quero confirmar suítes do imóvel',
                    parseVal: (v) => v != null && String(v).trim() !== '' && parseInt(v, 10) >= 0,
                    format: (v) => `${parseInt(v, 10)} suíte(s)`,
                });

            case 'INFO_VAGAS_AGG':
                return _respostaCampoCatalogo({
                    emoji: '🚗', nome: 'vagas de garagem', field: 'vagas',
                    waFrase: 'Olá Leandro! Quero confirmar vagas de garagem',
                    parseVal: (v) => v != null && String(v).trim() !== '' && parseInt(v, 10) >= 0,
                    format: (v) => `${parseInt(v, 10)} vaga(s)`,
                });

            case 'INFO_AREA_AGG':
                return _respostaCampoCatalogo({
                    emoji: '📐', nome: 'área útil (m²)', field: 'area',
                    waFrase: 'Olá Leandro! Quero confirmar a metragem do imóvel',
                    parseVal: (v) => v != null && String(v).trim() !== '',
                    format: (v) => `${String(v).replace(/\s*m\s*2\s*$/i, '').trim()} m²`,
                });

            case 'INFO_PRECO_AGG':
                return _respostaCampoCatalogo({
                    emoji: '💰', nome: 'preço', field: 'preco',
                    waFrase: 'Olá Leandro! Quero confirmar o valor do imóvel',
                    parseVal: (v, item) => parseFloat((v != null && v !== '' ? v : item.preco) || 0) > 0,
                    format: (v) => _formatPreco(v),
                });
            ...
            case 'ASSISTENTE_QUEM_E': {
                const estoqueInfo = _catalogoLoaded && _catalogoImoveis.length
                    ? `\n\n📊 Agora mesmo temos **${_catalogoImoveis.length} imóvel${_catalogoImoveis.length !== 1 ? 'is' : ''}** no catálogo — tudo em tempo real.`
                    : '';
                return {
                    text: `🤖 Sou o **Assistente Virtual** do **${KB.nome}**!\n\nPosso te ajudar com:\n\n🔍 **Buscar imóveis** — por bairro, tipo, quartos e preço\n_Ex: "Tem 2 quartos em Ipanema até 1M?"_\n\n📍 **Informações de bairros** — preços, perfil, transporte, Airbnb\n_Ex: "Me fala sobre a Barra da Tijuca"_\n\n💰 **Financiamento** — entrada, FGTS, taxas, bancos, MCMV\n_Ex: "Posso usar meu FGTS?"_\n\n📋 **Processo de compra** — passo a passo, documentação, custos\n_Ex: "Como funciona a compra de um imóvel?"_\n\n👤 **Sobre o Leandro** — experiência, CRECI, diferenciais, depoimentos\n\n📱 **Contato direto** — conectar com o corretor pelo WhatsApp${estoqueInfo}\n\n⚠️ Sou um assistente com roteiro fixo + busca no catálogo. Para negociação, visitas e valores exatos, o **${KB.nome}** (${KB.creci}) responde pessoalmente e rápido!`,
                    opts: [
            ...
            // ── INVESTIMENTO / AIRBNB — redireciona ao Leandro ──
            case 'CONSULTAR_LEANDRO':
                return {
                    text: `💼 Esse é um assunto que o **${KB.nome}** pode te ajudar com muito mais detalhes!\n\nCada caso é único e ele pode te orientar de forma personalizada sobre a melhor estratégia.`,
                    opts: [{ label: '📱 Falar com Leandro', action: () => window.open(KB.wa('Olá Leandro! Quero conversar sobre imóveis no Rio.'), '_blank') }, { label: '🏠 Ver catálogo', next: 'flow_catalogo' }, { label: '↩ Menu', next: 'inicio' }],
                };

            case 'ALUGUEL':
                return {
                    text: `🏠 **Sobre aluguéis no Rio:**\n\nO **${KB.nome}** pode te ajudar a encontrar o imóvel ideal, seja para morar ou alugar. Cada bairro tem faixas de preço diferentes!`,
                    opts: [{ label: '📱 Consultar Leandro', action: () => window.open(KB.wa('Olá Leandro! Quero saber sobre aluguéis no Rio.'), '_blank') }, { label: '↩ Menu', next: 'inicio' }],
                };
            ...
            case 'STUDIOS':
                return { text: `🏢 **Studios e kitnets:**\n\n• Tamanho: 20–50m²\n• Ótimos para quem busca praticidade\n\n**Preços:**\n• Copacabana: R$ 250–500k\n• Botafogo: R$ 280–480k\n• Barra: R$ 220–400k`, opts: [{ label: '📱 Ver studios com Leandro', action: () => window.open(KB.wa('Olá Leandro! Tenho interesse em studios.'), '_blank') }, { label: '↩ Menu', next: 'inicio' }] };
            ...
            case 'MELHOR_BAIRRO':
                return {
                    ...
                    opts: [
                        ...
                        { label: '📱 Falar com Leandro', action: () => window.open(KB.wa('Olá Leandro! Quero ajuda para escolher o melhor bairro para mim.'), '_blank') },
                        { label: '↩ Menu', next: 'inicio' },
                    ],
                };
    ...
    const FLOWS = {
        inicio: {
            msg: () => {
                const r = _resumoEstoque();
                const txtImv = r.total === 1 ? 'imóvel' : 'imóveis';
                const estoqueInfo = r ? `\n\n📊 Temos **${r.total} ${txtImv}** no catálogo.` : '';
                if (_ctx.userName) {
                    return `Olá de novo, **${_ctx.userName}**! 👋 Sou o assistente do Leandro.${estoqueInfo}\n\nComo posso te ajudar agora? (Use os botões abaixo ou digite)`;
                }
                return `Olá! 👋 Sou o assistente do **${KB.nome}**, corretor especialista no Rio de Janeiro.${estoqueInfo}\n\nPara começarmos bem, **como posso te chamar?**`;
            },
        },
        ...
        flow_zona_sul: {
            msg: () => {
                ...
                const txtImv = lista.length === 1 ? 'imóvel' : 'imóveis';
                const txtDisp = lista.length === 1 ? 'disponível' : 'disponíveis';
                const preview = lista.slice(0, 3).map(_renderCard).join('\n\n');
                return `🌊 **Zona Sul — ${lista.length} ${txtImv} ${txtDisp}**\n\n${preview}${lista.length > 3 ? `\n\n_...e mais ${lista.length - 3}_` : ''}`;
            },
            opts: () => [
        ...
        flow_barra: {
            msg: () => {
                ...
                const txtDisp = lista.length === 1 ? 'disponível' : 'disponíveis';
                const preview = lista.slice(0, 3).map(_renderCard).join('\n\n');
                return `🏙️ **Barra & Recreio — ${lista.length} ${txtImv} ${txtDisp}**\n\n${preview}${lista.length > 3 ? `\n\n_...e mais ${lista.length - 3}_` : ''}`;
            },
            opts: () => [
        ...
        flow_ate600: {
            msg: () => {
                ...
                const txtImv = lista.length === 1 ? 'imóvel' : 'imóveis';
                return lista.length
                    ? `💰 **${lista.length} ${txtImv} até R$ 600k:**\n\n${lista.slice(0, 3).map(_renderCard).join('\n\n')}`
        ...
        flow_investimento: { msg: () => gerarResposta({ intent: 'CONSULTAR_LEANDRO' })?.text || '', opts: () => gerarResposta({ intent: 'CONSULTAR_LEANDRO' })?.opts || [] },
    ...
    // ═══════════════════════════════════════════════════════
    const savedUserName = typeof localStorage !== 'undefined' ? localStorage.getItem('_lb_chat_userName') : null;
    ...
    const _ctx = {
        ...
        stage: savedUserName ? 'browsing' : 'waiting_name', // waiting_name → browsing → interested → ready_to_buy
        msgCount: 0,
        interests: new Set(),
    ...
    function _updateCtx(intentObj, resp) {
        ...
        const interestSignals = ['BUSCA_ESPECIFICA', 'INFO_BAIRRO', 'CATALOGO_GERAL', 'CONTAGEM_IMOVEIS', 'INFO_BANHEIROS_AGG', 'INFO_QUARTOS_AGG', 'INFO_SUITES_AGG', 'INFO_VAGAS_AGG', 'INFO_AREA_AGG', 'INFO_PRECO_AGG', 'DESTAQUES', 'MAIS_BARATO', 'MAIS_CARO'];
        ...
        if (['CONSULTAR_LEANDRO', 'ALUGUEL', 'VALORIZACAO'].includes(intentObj.intent)) _ctx.interests.add('consulta');
        if (['FGTS', 'ENTRADA'].includes(intentObj.intent)) _ctx.interests.add('compra');
    }
    ...
    // UNIFICAÇÃO DE DEVICE ID (Compartilhado com tracker.js)
    function _getDeviceId() {
        let id = localStorage.getItem('_lb_did');
        if (!id) {
            id = 'did_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 9);
            localStorage.setItem('_lb_did', id);
        }
        return id;
    }

    // SESSION ID PERSISTENTE POR DIA (Evita flood de sessões no admin)
    function _getOrCreateSessionId() {
        const today = new Date().toISOString().slice(0, 10); // "2024-05-20"
        let stored = localStorage.getItem('_lb_chat_sid');
        let sessionDate = localStorage.getItem('_lb_chat_sid_date');

        if (stored && sessionDate === today) {
            _sessionId = stored;
            return stored;
        }

        // Nova sessão para o novo dia
        const newSid = _getDeviceId() + '_' + Date.now().toString(36).slice(-5);
        localStorage.setItem('_lb_chat_sid', newSid);
        localStorage.setItem('_lb_chat_sid_date', today);
        _sessionId = newSid;
        return newSid;
    }

    // PERSISTÊNCIA DE APELIDO (Firestore + Local)
    async function _updateNickInSession(nick) {
        if (!nick) return;
        _ctx.userName = nick;
        try { localStorage.setItem('_lb_chat_userName', nick); } catch (e) { }

        if (typeof firebase !== 'undefined' && firebase.apps.length) {
            const db = firebase.firestore();
            const did = _getDeviceId();
            // Salva na coleção global de visitantes
            db.collection('visitantes_nicks').doc(did).set({
                nick: nick,
                lastSeen: firebase.firestore.FieldValue.serverTimestamp(),
                updatedAt: new Date().toISOString()
            }, { merge: true }).catch(e => console.error("Err Nick:", e));
        }
    }
    ...
    // LOGGING REFORÇADO (Com Nick, SessionId e info de dispositivo)
    function _log(event, data = {}) {
        try {
            if (typeof firebase === 'undefined' || !firebase.apps || !firebase.apps.length) return;
            const db = firebase.firestore();

            if (!_sessionId) _getOrCreateSessionId();

            // Usa detecção de dispositivo do tracker se disponível
            const di = window._lbDetectDevice ? {
                os: window._lbDetectDevice.getOS(),
                deviceType: window._lbDetectDevice.getType(),
                browser: window._lbDetectDevice.getBrowser(),
                deviceIcon: window._lbDetectDevice.getIcon(),
            } : { os: '—', deviceType: '—', browser: '—', deviceIcon: '💻' };

            db.collection('chat_logs').add({
                sessionId: _sessionId,
                deviceId: _getDeviceId(),
                nickName: _ctx.userName || 'Visitante',
                displayLabel: _ctx.userName ? `👤 ${_ctx.userName}` : '👤 Visitante',
                event,
                ...
                msgCount: _msgCount,
                chatPath: _chatPath.slice(-6).join('→'),
                // Info de dispositivo
                os: di.os,
    ...
    function createWidget() {
        ...
        style.id = '_lb-chat-styles-v9';
        style.textContent = `
        /* ── Base ── */
        #lb-contact-dock, #lb-chat-window {
            visibility: visible !important;
            box-sizing: border-box;
            font-family: 'Montserrat', sans-serif;
        }
        #lb-contact-dock div:empty { display: none; }

        /* ── Dock de botões (canto inferior ESQUERDO) ── */
        #lb-contact-dock {
            position: fixed;
        ...
        #lb-chat-btn {
            width: 56px; height: 56px; border-radius: 50%;
            background: linear-gradient(165deg, #2563eb 0%, #0f172a 100%);
            border: 2px solid rgba(59, 130, 246, 0.55); cursor: pointer;
            display: flex; align-items: center; justify-content: center;
            box-shadow: 0 4px 22px rgba(37, 99, 235, 0.38);
            transition: transform .25s, box-shadow .25s;
            flex-shrink: 0; position: relative;
        }
        #lb-chat-btn:hover { transform: scale(1.06); box-shadow: 0 8px 28px rgba(59, 130, 246, 0.48); }
        #lb-chat-btn svg { width: 26px; height: 26px; transition: transform .3s; }
        #lb-chat-btn.open svg { transform: scale(0.9); }

        /* ── Tooltip labels (aparecem ao hover nos botões) ── */
        .lb-fab-label {
            ...
            box-shadow: 0 6px 20px rgba(0,0,0,0.5); border: 1px solid rgba(59,130,246,0.22);
            font-family: inherit; z-index: 1;
        }
        #lb-wa-dock:hover .lb-fab-label,
        #lb-chat-btn:hover .lb-fab-label {
            opacity: 1; transform: translateY(-50%) translateX(0);
        }

        /* ── Ponto de notificação no botão ── */
        .lb-notif-dot {
            position: absolute; top: 3px; right: 3px;
            width: 12px; height: 12px; background: #ef4444; border-radius: 50%;
            border: 2px solid #0c1520; animation: lb-pulse-dot 2s ease-in-out infinite;
        }

        /* ════════════════════════════════════════
           JANELA DO CHAT
           Posição: ACIMA dos botões, à esquerda
           ════════════════════════════════════════ */
        #lb-chat-window {
            position: fixed;
            /* Fica acima dos botões (dock height ~120px + margem) */
            bottom: calc(max(1.1rem, env(safe-area-inset-bottom, 0px)) + 136px);
            left: max(1rem, env(safe-area-inset-left, 0px));
            right: auto;
            ...
            z-index: 10039;
            overflow: hidden;
            /* Animação: escondido por padrão */
            transform: scale(.88) translateY(12px);
            opacity: 0;
            pointer-events: none;
            transition: transform .3s cubic-bezier(.22,1,.36,1), opacity .28s;
            /* Garante que não aparece fora do chat */
            visibility: hidden;
        }
        #lb-chat-window.open {
        ...
        /* Evita rótulo do FAB sobrepondo a janela / campo de texto */
        body:has(#lb-chat-window.open) #lb-contact-dock .lb-fab-label {
            opacity: 0 !important;
        ...
        /* ── Header do chat ── */
        .lb-chat-header {
            background: linear-gradient(180deg, rgba(30,58,95,.6) 0%, rgba(16,26,40,.95) 100%);
        ...
        .lb-chat-avatar {
            ...
            background: linear-gradient(145deg,#2563eb,#1e3a5f);
            display: flex; align-items: center; justify-content: center; font-size: 1rem;
            flex-shrink: 0; position: relative;
        }
        .lb-chat-avatar::after {
            content:''; position:absolute; bottom:1px; right:1px;
            width:9px; height:9px; background:#22c55e; border-radius:50%; border:2px solid #101a28;
        }
        .lb-chat-name { font-size:.88rem;font-weight:700;color:#f1f5f9;line-height:1.2; }
        .lb-chat-subtitle { font-size:.68rem;color:rgba(226,232,240,.5);margin-top:.08rem; }
        .lb-chat-status { font-size:.7rem;color:#4ade80;display:flex;align-items:center;gap:.3rem;margin-top:.12rem; }
        .lb-chat-status-dot { width:6px;height:6px;background:#4ade80;border-radius:50%;display:inline-block;animation:lb-pulse-dot 2s infinite; }
        .lb-badge-estoque { display:inline-flex;align-items:center;gap:.2rem;background:rgba(59,130,246,.14);border:1px solid rgba(59,130,246,.32);color:#93c5fd;font-size:.58rem;font-weight:700;padding:.08rem .4rem;border-radius:99px;margin-left:.3rem; }
        .lb-header-actions { margin-left:auto;display:flex;gap:.25rem; }
        .lb-header-btn { background:rgba(255,255,255,.07);border:none;color:rgba(255,255,255,.5);width:28px;height:28px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:.7rem;transition:all .2s; }
        ...
        .lb-chat-messages::-webkit-scrollbar { width: 3px; }
        .lb-chat-messages::-webkit-scrollbar-thumb { background: rgba(59,130,246,.3); border-radius: 2px; }

        /* Mensagens */
        .lb-msg { max-width: 93%; padding: .6rem .85rem; border-radius: 14px; font-size: .82rem; line-height: 1.5; color: #e8edf5; animation: lb-msgIn .22s ease both; }
        @keyframes lb-msgIn { from{opacity:0;transform:translateY(7px);}to{opacity:1;transform:translateY(0);} }
        .lb-msg.bot { background:rgba(30,58,95,.35);border:1px solid rgba(59,130,246,.18);border-bottom-left-radius:4px;align-self:flex-start; }
        .lb-msg.bot strong { color:#93c5fd; }
        .lb-msg.user { background:rgba(37,99,235,.28);border:1px solid rgba(59,130,246,.32);border-bottom-right-radius:4px;align-self:flex-end;text-align:right; }
        .lb-msg-time { font-size:.6rem;color:rgba(226,232,240,.35);margin-top:.18rem;display:block; }

        /* Typing */
        .lb-typing { display:flex;align-items:center;gap:4px;padding:.5rem .8rem;background:rgba(30,58,95,.3);border:1px solid rgba(59,130,246,.15);border-radius:14px;border-bottom-left-radius:4px;align-self:flex-start;animation:lb-msgIn .2s ease; }
        .lb-typing span { width:5px;height:5px;background:#60a5fa;border-radius:50%;animation:lb-typingDot 1.2s ease-in-out infinite; }
        ...
        /* ── Chips de atalho (apenas visíveis quando o chat está ABERTO) ── */
        .lb-chips {
            display: flex; flex-wrap: wrap; gap: .28rem;
            padding: .3rem .75rem .38rem;
            flex-shrink: 0;
            border-top: 1px solid rgba(59,130,246,.08);
            background: #101a28;
            /* Escondido por padrão — só aparece dentro da janela aberta */
        }
        .lb-chip { background:rgba(59,130,246,.1);border:1px solid rgba(59,130,246,.2);border-radius:99px;padding:.2rem .6rem;color:#93c5fd;font-size:.67rem;font-weight:500;cursor:pointer;transition:all .18s;white-space:nowrap; }
        .lb-chip:hover { background:rgba(59,130,246,.2);border-color:rgba(59,130,246,.42); }

        /* ── Input de texto (DENTRO do chat, nunca visível fora) ── */
        .lb-input-area {
        ...
        .lb-text-input { flex:1;background:rgba(255,255,255,.06);border:1px solid rgba(59,130,246,.2);border-radius:18px;padding:.44rem .85rem;color:#e8edf5;font-size:.8rem;font-family:inherit;outline:none;transition:border-color .2s,background .2s; }
        .lb-text-input:focus { border-color:rgba(59,130,246,.5);background:rgba(255,255,255,.07); }
        .lb-text-input::placeholder { color:rgba(226,232,240,.3); }
        .lb-send-btn { width:30px;height:30px;border-radius:50%;background:linear-gradient(145deg,#2563eb,#1e3a5f);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;font-size:.72rem;flex-shrink:0;transition:transform .18s,box-shadow .18s; }
        .lb-send-btn:hover { transform:scale(1.08);box-shadow:0 3px 12px rgba(59,130,246,.42); }

        /* Footer */
        .lb-chat-footer { padding:.3rem .75rem;border-top:1px solid rgba(59,130,246,.07);font-size:.6rem;color:rgba(226,232,240,.35);text-align:center;flex-shrink:0;background:#0c1520; }
        ...
        @media (max-width: 600px) {
            ...
            #lb-chat-window {
                /* No mobile, ocupa mais largura e fica mais alto */
                left: 0.5rem;
                right: 0.5rem;
                width: auto;
                bottom: calc(max(0.75rem, env(safe-area-inset-bottom, 0px)) + 132px);
                max-height: min(520px, calc(100dvh - 150px));
                border-radius: 16px;
            }
            #lb-wa-dock, #lb-chat-btn { width: 52px; height: 52px; }
            #lb-wa-dock .fab { font-size: 1.3rem; }
            #lb-chat-btn svg { width: 23px; height: 23px; }
            /* Tooltips: oculta no mobile (tela pequena) */
        ...
        @media (max-width: 380px) {
            #lb-chat-window {
                left: 0.35rem;
                right: 0.35rem;
                bottom: calc(max(0.6rem, env(safe-area-inset-bottom, 0px)) + 124px);
                max-height: min(480px, calc(100dvh - 140px));
                border-radius: 14px;
            }
            .lb-chat-header { padding: .7rem .85rem; }
            .lb-chat-name { font-size: .82rem; }
            .lb-msg { font-size: .78rem; }
        }

        /* Painel Admin com bottom-nav: ajuste para não colidir */
        @media (max-width: 768px) {
            body.admin-page #lb-contact-dock { display: none !important; }
            body.admin-page #lb-chat-window { display: none !important; }
        }
        `;
        ...
        btn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 8V4H8"/><rect x="4" y="8" width="16" height="12" rx="2" ry="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/></svg><span class="lb-notif-dot"></span><span class="lb-fab-label">Assistente Virtual</span>`;
        ...
        // column-reverse: first appended = bottom
        dock.appendChild(wa);
        ...
        win.innerHTML = `
            ...
            <div class="lb-chat-footer">Posso cometer erros! · ${KB.nome} · ${KB.creci}</div>
        `;
    ...
    function handleText() {
        ...
        // ── ONBOARDING: CAPTURA E PERSISTÊNCIA DO APELIDO ──
        if (_ctx.stage === 'waiting_name') {
            _ctx.stage = 'browsing';
            const clean = text.toLowerCase().trim();
            if (clean.length < 25 && !clean.includes('quero') && !clean.includes('imovel') && !_extractBairro(text)) {
                const nameMatch = text.match(/(?:meu nome e|me chamo|sou o|sou a|pode me chamar de)?\s*([a-zA-ZÀ-ÿ]{2,})/i);
                let capturedNick = nameMatch ? nameMatch[1] : text;
                capturedNick = capturedNick.charAt(0).toUpperCase() + capturedNick.slice(1).split(' ')[0]; // Apenas primeiro nome

                _updateNickInSession(capturedNick); // <-- Salva no Firestore e local

                showTypingThen(() => {
                    addBotMsg(`Prazer, **${capturedNick}**! 😊 O que você está procurando hoje?\n\nPode me descrever livremente ou usar os atalhos azuis na base do chat. 👇`);
                    _log('chat_identificacao', { nick: capturedNick });
                });
                return;
            }
        }
        ...
        const stageHint = _ctx.stage === 'ready_to_buy'
            ...
            : '';
        showTypingThen(() => {
            const botResponse = `Boa pergunta${_ctx.userName ? ', ' + _ctx.userName : ''}! 🤔 O **${KB.nome}** conhece cada detalhe técnico e pode te responder com precisão.${stageHint}\n\nPosso te conectar agora — ele costuma responder rápido no WhatsApp!\n\n💡 **Dica:** você pode buscar por bairro ("Imóveis em Ipanema"), tipo ("Quero cobertura") ou características ("Com piscina").`;
            ...
            setTimeout(() => addOpts([
                ...
                ...(_ctx.lastBairro ? [{ label: `📍 Imóveis em ${_ctx.lastBairro}`, next: `flow_${_ctx.lastBairroKey || 'bairros'}` }] : []),
                { label: '🏠 Ver catálogo completo', next: 'flow_catalogo' },
                { label: '↩ Menu principal', next: 'inicio' },
            ]), 400);
        });
    }
    ...
    function _buildContextualFallback(text) {
        ...
        const vagueContinue = t.length <= 56 && matchAny(t, [
            'e agora', 'e dai', 'e daí', 'continua', 'continuar', 'proximo', 'próximo',
            'proximo passo', 'próximo passo', 'mais detalhes', 'me explica', 'explica direito',
            'nao entendi', 'não entendi', 'como assim', 'beleza e', 'ok e', 'sim e',
            'aham', 'uhum', 'segui', 'seguindo', 'e depois', 'e entao', 'e então',
            'o que mais', 'que mais', 'manda ver', 'pode crer', 'e ai', 'e aí',
        ]);

        if (vagueContinue && _ctx.lastBairroKey) {
            const r = gerarResposta({ intent: 'INFO_BAIRRO', bairro: _ctx.lastBairroKey });
            if (r && r.text) {
                return {
                    text: `Beleza — seguindo no papo de **${_ctx.lastBairro || 'esse bairro'}**:\n\n${r.text}`,
                    opts: r.opts,
                };
            }
        }

        if (vagueContinue && (_ctx.lastFlow === 'flow_financiamento' || ['FINANCIAMENTO', 'FGTS', 'ENTRADA', 'JUROS', 'APROVACAO', 'MCMV'].includes(_ctx.lastIntent || ''))) {
            return {
                text: `Fechando o raciocínio: no financiamento entram **entrada**, **parcela**, **taxas** e (quando liberado) **FGTS**. Me diz o que você quer cravar que eu te encaminho:`,
                opts: [
                    { label: '🏦 FGTS e entrada', next: 'flow_financiamento' },
                    { label: '📋 Documentação', next: 'flow_financiamento' },
                    { label: '📱 Falar com o Leandro', action: () => window.open(KB.wa('Olá Leandro! Estou no assistente do site e quero continuar sobre financiamento.'), '_blank') },
                    { label: '↩ Menu', next: 'inicio' },
                ],
            };
        }

        if (vagueContinue && _ctx.lastFlow && FLOWS[_ctx.lastFlow] && _ctx.lastFlow !== 'inicio') {
            return {
                text: `Quer **rever as opções** desse passo ou ir direto pro **WhatsApp** com o Leandro?`,
                opts: [
                    { label: '🔄 Ver opções de novo', next: _ctx.lastFlow },
                    { label: '📱 WhatsApp', action: () => window.open(KB.wa('Olá Leandro! Estou no assistente do site e quero continuar a conversa.'), '_blank') },
                    { label: '🏠 Catálogo', next: 'flow_catalogo' },
        ...
        // Reação curta positiva com bairro em contexto → aprofunda no mesmo bairro
        if (_ctx.lastBairroKey && t.length < 28 && matchAny(t, ['legal', 'interessante', 'bacana', 'show', 'gostei', 'lindo', 'bonito', 'massa', 'top', 'demais', 'sinistro', 'brabo', 'brabissimo', 'caraca', 'nossa', 'irado'])) {
            const r = gerarResposta({ intent: 'INFO_BAIRRO', bairro: _ctx.lastBairroKey });
            if (r) return r;
        }
    ...
    function _buildModalContextResponse(imovel, text) {
        const t = normFull(text);
        let msg = `Opa! 🏠 Analisando o **${imovel.titulo || 'imóvel atual'}**:\n\n`;
        let answered = false;

        if (matchAny(t, ['quarto', 'quartos'])) { msg += `🛏️ Ele possui **${imovel.quartos || 0} quarto(s)**.\n`; answered = true; }
        if (matchAny(t, ['suite', 'suites'])) { msg += `🛀 Ele possui **${imovel.suites || 0} suíte(s)**.\n`; answered = true; }
        if (matchAny(t, ['banheiro', 'banheiros', 'lavabo'])) {
            const b = imovel.banheiros;
            const has = b != null && String(b).trim() !== '' && parseInt(b, 10) >= 0;
            msg += has ? `🚿 Constam **${parseInt(b, 10)} banheiro(s)** no cadastro.\n` : `🚿 O número de **banheiros** não está preenchido no cadastro online — o Leandro confirma na ficha.\n`;
            answered = true;
        }
        if (matchAny(t, ['vaga', 'vagas', 'garagem']) && imovel.vagas) { msg += `🚗 Possui **${imovel.vagas} vaga(s)** de garagem.\n`; answered = true; }
        if (matchAny(t, ['area', 'tamanho', 'metros', 'm2'])) { msg += `📏 Tem área útil de **${imovel.area}m²**.\n`; answered = true; }
        if (matchAny(t, ['condominio'])) {
            if (imovel.condominio) msg += `🏢 O valor do condomínio é aproximadamente **R$ ${imovel.condominio}**.\n`;
            else msg += `🏢 Consulte o Leandro para saber o valor exato do condomínio.\n`;
            answered = true;
        }
        if (matchAny(t, ['iptu'])) {
            if (imovel.iptu) msg += `📄 O valor do IPTU é cerca de **R$ ${imovel.iptu}**.\n`;
            else msg += `📄 Consulte o Leandro para saber o valor exato do IPTU.\n`;
            answered = true;
        }
        if (matchAny(t, ['preco', 'valor', 'custa'])) {
            const p = parseFloat(imovel.preco || 0);
            if (p > 0) msg += `💰 O valor atual é **R$ ${p.toLocaleString('pt-BR')}**.\n`;
            else msg += `💰 O valor está sob consulta.\n`;
            answered = true;
        }

        if (matchAny(t, ['agendar', 'visita', 'ver', 'visitar'])) {
            return {
                text: `📅 Que ótimo! Você quer visitar o **${imovel.titulo}**.\nO Leandro Bomfim agenda rapidamente e acompanha pessoalmente!`,
                opts: [
                    { label: '📱 Agendar no WhatsApp', action: () => window.open(KB.wa(`Olá Leandro! Quero agendar uma visita para o imóvel: *${imovel.titulo}* (ID: ${imovel.id}). Pode me passar os horários disponíveis?`), '_blank') },
                    { label: '↩ Menu', next: 'inicio' }
                ]
            };
        }
        ...
        if (!answered) {
            msg += `Ele fica localizado em **${imovel.bairro}** e custa **R$ ${parseFloat(imovel.preco || 0).toLocaleString('pt-BR')}**.\n\nSe tiver dúvidas mais específicas sobre reforma, negociação ou documentação, fale com o Leandro.`;
        }

        return {
            text: msg,
            opts: [
                { label: '📱 Falar sobre este imóvel', action: () => window.open(KB.wa(`Olá Leandro! Tenho interesse no imóvel *${imovel.titulo}* (ID: ${imovel.id}). Pode me dar mais detalhes específicos?`), '_blank') },
                { label: '↩ Menu', next: 'inicio' }
            ]
        };
    }
    ...
    function addOpts(opts) {
        ...
        opts.forEach(opt => {
            const btn = document.createElement('button');
            btn.style.cssText = 'background:rgba(59,130,246,.1);border:1px solid rgba(59,130,246,.28);border-radius:99px;padding:6px 14px;color:#93c5fd;font-size:.76rem;font-weight:600;cursor:pointer;transition:all .2s;white-space:nowrap;font-family:inherit;';
            btn.textContent = opt.label;
            btn.addEventListener('mouseenter', () => { btn.style.background = 'rgba(59,130,246,.22)'; btn.style.borderColor = 'rgba(59,130,246,.5)'; });
            btn.addEventListener('mouseleave', () => { btn.style.background = 'rgba(59,130,246,.1)'; btn.style.borderColor = 'rgba(59,130,246,.28)'; });
