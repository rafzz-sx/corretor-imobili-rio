...
//  v2.1 — Correções mobile: urgency bar, live badge, z-index
// ============================================================
...
(function () {
    ...
    function injectAdStyles() {
        ...
        .lb-ad-preco {
            font-size:1.1rem;font-weight:800;color:#3498db;
            margin-bottom:.7rem;font-family:'Playfair Display',serif;
        }
        .lb-ad-cta {
        ...
        .lb-ad-dot {
            ...
            background:rgba(255,255,255,.15);cursor:pointer;transition:all .25s;
        }
        .lb-ad-dot.active { background:#3498db;width:14px;border-radius:3px; }

        /* ── Animação pulse compartilhada ── */
        @keyframes livePulse { 0%,100%{opacity:1;transform:scale(1);}50%{opacity:.5;transform:scale(1.4);} }

        /* ── Barra de urgência ──
           PROBLEMA ORIGINAL: top:80px fixo quebrava no mobile (header tem 60px).
           CORREÇÃO: usa variável CSS que muda conforme breakpoint.
        ── */
        #lb-urgency-bar {
            position: fixed;
            top: 80px; /* desktop: header = 80px */
            left: 0; right: 0;
            ...
            z-index: 95; /* abaixo do header (100), acima do conteúdo */
        ...
        #lb-urgency-bar a {
            ...
            background:rgba(52,152,219,.2);border:1px solid rgba(52,152,219,.3);
        ...
        #lb-urgency-close {
            ...
            cursor:pointer;font-size:.65rem;padding:.25rem;line-height:1;
        }
        #lb-urgency-close:hover { color:rgba(255,255,255,.7); }

        /* ── Contador ao vivo ──
           PROBLEMA ORIGINAL: top:90px empurrava para cima do header no mobile.
           CORREÇÃO: usa z-index menor que header e top correto por breakpoint.
        ── */
        #lb-live-badge {
            position: fixed;
            top: 88px; /* desktop: logo abaixo do header */
            right: 1rem;
        ...
        @media (max-width: 768px) {
            /* No mobile o header tem 60px — ajusta tudo que depende dele */
            #lb-urgency-bar {
                top: 60px;
                font-size: .67rem;
                padding: .32rem 2.2rem .32rem .7rem;
            }
            #lb-live-badge {
        ...
        @media (max-width: 380px) {
            #lb-urgency-bar  { font-size: .6rem; }
            #lb-live-badge   { font-size: .6rem; }
        }
        `;
    ...
    function buildAdEl(ad) {
        ...
        el.innerHTML = `
            <div class="lb-ad-image-wrap">
                <img src="${ad.img}" alt="${ad.titulo}" loading="lazy"
                     onerror="this.src='https://via.placeholder.com/280x130/1a1a2e/fff?text=Imóvel'">
    ...
    function showAd(index) {
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => { if (_adEl) _adEl.classList.add('show'); });
        });
    ...
    function initUrgencyBar() {
        ...
        bar.innerHTML = `
            <span class="lb-live-dot" style="width:6px;height:6px;background:#22c55e;border-radius:50%;
                box-shadow:0 0 5px #22c55e;flex-shrink:0;animation:livePulse 1.5s infinite;display:inline-block;"></span>
            <span>Novo imóvel em <strong>Ipanema</strong> esta semana!</span>
        ...
        // Auto-fecha em 8s no mobile para não atrapalhar a navegação
        ...
        function updateCount() {
            ...
            if (typeof firebase !== 'undefined' && firebase.apps && firebase.apps.length) {
                try {
                    ...
                    const el = document.getElementById('lb-live-count'); if(el) el.textContent = base; badge.classList.add('show');
                }
            } else {
                const el = document.getElementById('lb-live-count'); if(el) el.textContent = base; badge.classList.add('show');
            }
