            ...
            firebase.firestore().collection('config').doc('site').get().then(function(doc) {
                ...
                if (cfg.heroTitulo) {
                    ...
                    if (titleEl) {
                        ...
                        var l1, l2, l3;
                        if (words.length <= 3) {
                            // 1-3 palavras: uma por linha
                            l1 = words[0] || '';
                            l2 = words[1] || '';
                            l3 = words[2] || '';
                        } else if (words.length === 4) {
                            // "Transformando Sonhos em Endereços" → linha1=palavra1, linha2=palavra2, linha3=palavra3+4
                            l1 = words[0];
                            l2 = words[1];
                            l3 = words[2] + ' ' + words[3];
                        } else {
                            // 5+ palavras: divide em 3 grupos
                            var a = Math.ceil(words.length / 3);
